...
func TestElastic_Methods(t *testing.T) {
	...
	t.Run("Unwrap", func(t *testing.T) {
		assert.Assert(t, mixed2.Unwrap().EqualFunc(und.Defined(option.Options[string]{option.None[string](), option.Some("bar")}), option.EqualOptions))
		assert.Assert(t, null.Unwrap().EqualFunc(und.Null[option.Options[string]](), option.EqualOptions))
		assert.Assert(t, undefined.Unwrap().EqualFunc(und.Undefined[option.Options[string]](), option.EqualOptions))
	})
}
